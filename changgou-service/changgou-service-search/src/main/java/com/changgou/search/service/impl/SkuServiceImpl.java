...
public class SkuServiceImpl implements SkuService {
    ...
    @Autowired
    private ElasticsearchTemplate elasticsearchTemplate;//执行索引库的增删改查
    ...
    public Map<String, Object> search(Map<String, String> searchMap) {
        ...
        nativeSearchQueryBuilder.addAggregation(AggregationBuilders
                .terms("skuCategorygroup").field("categoryName").size(50)); //设置分组条件  商品分类

        nativeSearchQueryBuilder.addAggregation(AggregationBuilders
                .terms("skuBrandgroup").field("brandName").size(50));  //设置分组条件  商品品牌

        nativeSearchQueryBuilder.addAggregation(AggregationBuilders
                .terms("skuSpecgroup").field("spec.keyword").size(100));   //设置分组条件  商品的规格

        //设置高亮条件
        nativeSearchQueryBuilder.withHighlightFields(
                new HighlightBuilder.Field("name"));//指定高亮域
        ...
        nativeSearchQueryBuilder.
                withQuery(QueryBuilders.multiMatchQuery(keywords,"name","brandName","categoryName"));
        ...
        //========================过滤查询 开始=====================================
        //组合查询 boolquery  must ,must_Not ,should

        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();//构建对象

        //  过滤查询的条件设置   分类的条件
        String category = searchMap.get("category");
        if(!StringUtils.isEmpty(category)) {
            boolQueryBuilder.filter(QueryBuilders.termQuery("categoryName", category));//分类
        }
    ...
    private Map<String, Set<String>> getStringSetMap(StringTerms stringTermsSpec) {
        ...
        Map<String, Set<String>> specMap = new HashMap<String, Set<String>>();//合并后的数据
        ...
        //循环遍历
        //将map数据 合并成 Map<String ,Set<String>
        for (String specjson : specList) {
            ...
            for (Map.Entry<String, String> entry : map.entrySet()) {
                ...
                Set<String> specValues = specMap.get(key); //获取当前规格名字对应的规格数据
                if (specValues == null) {
                    specValues = new HashSet<String>();//没有new
                }
                //将当前规格加入到集合中
