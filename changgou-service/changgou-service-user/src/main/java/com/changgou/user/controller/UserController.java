...
public class UserController {
    ...
    public Result login(String username, String password, HttpServletResponse response, HttpServletRequest request){
        ...
        if (BCrypt.checkpw(password, user.getPassword())){
            ...
            //密码匹配，登录成功
            return new Result(true, StatusCode.OK, "登录成功", token);//把令牌token作为参数给用户
        }


        //密码匹配失败，账户或者密码失败
        return new Result(false, StatusCode.LOGINERROR, "账户或者用户名有误");
    }
     ...
     */
    @GetMapping
    @PreAuthorize("hasAnyRole('user')")//指定权限
